...
@section('title', 'Reset Kata Sandi') {{-- Judul halaman untuk tag <title> --}}
...
<div class="min-h-screen flex items-center justify-center bg-gray-50 px-4 py-8 sm:px-6 lg:px-8"> {{-- Warna latar belakang terang --}}
    <div class="max-w-md w-full bg-white shadow-xl rounded-lg p-8 md:p-10 relative border border-gray-200"> {{-- Card utama putih dengan shadow lebih menonjol dan border tipis --}}
        ...
        <h2 class="text-4xl font-extrabold text-gray-900 mb-4 text-center mt-8 tracking-tight">Reset Kata Sandi Anda 🔄</h2> {{-- Judul lebih besar, bold, dan tracking lebih rapat --}}
        <p class="text-gray-600 mb-8 text-center leading-relaxed">
            Masukkan alamat email dan kata sandi baru Anda untuk mereset akun.
        </p>
        ...
        <form method="POST" action="{{ route('password.store') }}" class="space-y-6"> {{-- Tambah space-y untuk jarak antar elemen form --}}
            @csrf
            ...
            <div>
                ...
                <input id="email" class="block w-full px-4 py-2 border border-gray-300 rounded-md shadow-sm placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" type="email" name="email" value="{{ old('email', $request->email) }}" required autofocus placeholder="contoh@email.com" />
                @error('email')<p class="text-red-500 text-xs italic mt-2 text-left">{{ $message }}</p>@enderror
            </div>

            <div>
                <label for="password" class="block text-left text-sm font-medium text-gray-700 mb-1">Kata Sandi Baru</label> {{-- Label lebih deskriptif --}}
            ...
            <div>
                <label for="password_confirmation" class="block text-left text-sm font-medium text-gray-700 mb-1">Konfirmasi Kata Sandi Baru</label> {{-- Label lebih deskriptif --}}
