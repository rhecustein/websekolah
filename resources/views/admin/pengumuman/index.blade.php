                    ...
                    <form action="{{ route('admin.pengumuman.index') }}" method="GET" class="mb-6">
                        <div class="grid grid-cols-1 gap-4 md:grid-cols-3">
                            <div class="md:col-span-2">
                                <input type="text" name="search" placeholder="Cari judul pengumuman..." class="w-full border-gray-300 rounded-md shadow-sm focus:ring-sky-500 focus:border-sky-500" value="{{ request('search') }}">
                            </div>
                             <div>
                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                @forelse ($pengumumans as $pengumuman)
                                    <tr>
                                        ...
                                        <td class="px-6 py-4 text-sm text-gray-500 whitespace-nowrap">{{ $pengumuman->user->name }}</td>
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <span class="inline-flex px-2 text-xs font-semibold leading-5 rounded-full 
                                                @if($pengumuman->status == 'published') bg-green-100 text-green-800 @endif
                                                @if($pengumuman->status == 'draft') bg-yellow-100 text-yellow-800 @endif
                                                @if($pengumuman->status == 'archived') bg-gray-100 text-gray-800 @endif
                                            ">
                                                {{ ucfirst($pengumuman->status) }}
                                            </span>
                                        </td>
                                        <td class="px-6 py-4 text-sm text-gray-500 whitespace-nowrap">{{ $pengumuman->published_at ? $pengumuman->published_at->format('d M Y') : '-' }}</td>
                                        <td class="px-6 py-4 text-sm font-medium text-right whitespace-nowrap">
                                            <div class="flex items-center justify-end space-x-2">
                                                <a href="{{ route('admin.pengumuman.edit', $pengumuman->id) }}" class="text-indigo-600 hover:text-indigo-900">Edit</a>
