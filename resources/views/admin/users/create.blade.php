            ...
            <div class="bg-white rounded-xl shadow-lg border border-slate-200"> {{-- Menggunakan kelas shadow-lg, rounded-xl, dan border --}}
                <form action="{{ route('admin.users.store') }}" method="POST">
                    ...
                    <div class="p-6 md:p-8 space-y-6"> {{-- Padding disesuaikan, space-y-6 tetap --}}
                        ...
                        <div class="p-6 bg-white rounded-xl shadow-lg border">
                            ...
                            <div class="space-y-4">
                                <div>
                                    ...
                                    <input type="text" name="name" id="name" value="{{ old('name') }}" class="block w-full mt-1 border-slate-300 rounded-lg shadow-sm focus:ring-sky-500 focus:border-sky-500 sm:text-sm" placeholder="Masukkan nama lengkap pengguna" required>
                                ...
                                <div>
                                    ...
                                    <input type="email" name="email" id="email" value="{{ old('email') }}" class="block w-full mt-1 border-slate-300 rounded-lg shadow-sm focus:ring-sky-500 focus:border-sky-500 sm:text-sm" placeholder="Masukkan alamat email pengguna" required>
                        ...
                        {{-- Kartu Kata Sandi --}}
                        <div class="p-6 bg-white rounded-xl shadow-lg border">
                            <h3 class="text-lg font-bold text-slate-800 border-b border-slate-200 pb-3 mb-4">Kata Sandi</h3>
                            <div class="grid grid-cols-1 gap-4 md:grid-cols-2"> {{-- Gap disesuaikan --}}
                                <div>
                                    <label for="password" class="block text-sm font-medium text-slate-700">Password <span class="text-red-500">*</span></label>
                                    <input type="password" name="password" id="password" class="block w-full mt-1 border-slate-300 rounded-lg shadow-sm focus:ring-sky-500 focus:border-sky-500 sm:text-sm" placeholder="Masukkan password" required>
                                ...
                                <div>
                                    ...
                                    <input type="password" name="password_confirmation" id="password_confirmation" class="block w-full mt-1 border-slate-300 rounded-lg shadow-sm focus:ring-sky-500 focus:border-sky-500 sm:text-sm" placeholder="Konfirmasi password" required>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="p-6 bg-white rounded-xl shadow-lg border">
                            ...
                            <div class="mt-2 space-y-2">
                                @forelse ($roles as $role)
                                    <div class="flex items-center">
                                        {{-- old('roles') akan mengembalikan array, jadi kita perlu mengecek apakah id role ada di dalamnya --}}
                                        <input id="role-{{ $role->id }}" name="roles[]" type="checkbox" value="{{ $role->id }}" class="w-4 h-4 rounded text-sky-600 border-slate-300 focus:ring-sky-500" {{ (is_array(old('roles')) && in_array($role->id, old('roles'))) ? 'checked' : '' }}>
